...
parser.getText().then(res => {
  ...
  // 2. Extract 540 Email rows sequentially
  const emailRows = [];
  ...
  console.log(`Parsed ${emailRows.length} email rows.`);

  // Combine by index (SlNo - 1)
  const matchedStudents = studentList.map((s, idx) => {
    const e = emailRows[idx] || {};
    return {
      slNo: s.slNo,
      regNo: s.regNo,
      name: s.name,
  ...
  const yakshasri = matchedStudents.find(x => x.regNo === '24B11AI283');
  console.log("\n=== Verified Profile for 24B11AI283 (ANEPU YAKSHASRI) ===");
  console.log(JSON.stringify(yakshasri, null, 2));

  // Verify ADAPA SOWJANYA PRIYA (SlNo 1, 24B11AI001)
  const sowjanya = matchedStudents.find(x => x.regNo === '24B11AI001');
  console.log("\n=== Verified Profile for 24B11AI001 (ADAPA SOWJANYA PRIYA) ===");
  console.log(JSON.stringify(sowjanya, null, 2));
